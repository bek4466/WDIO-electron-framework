...
const logClient = new LogClient("e2e:PasswordVerifyField>POM");
import { browser } from "@wdio/globals";
import { Key } from "webdriverio";

export class ProtectProjectPasswordFieldConfirm extends MatButton {
  constructor(
  ...
  public async click(): Promise<void> {
    await allure.step(
      ...
      async () => {
        await allure.screenshot(
          this.app,
          "Protect Project password field confirmation before click action "
        );
        await super.click();
      }
    );
    await allure.screenshot(
      this.app,
      "After protect project password field confirmation was clicked"
    );
  }

  public async clearPassword() {
    await (await browser.$(locators.passwordFieldConfirm)).click();
  ...
  public async setPassword(password: string): Promise<void> {
    ...
    let passwordFieldExists = await (await browser.$(locators.passwordFieldConfirm)).isExisting();
    try {
      if (passwordFieldExists) {
        await browser.$(locators.passwordFieldConfirm).setValue(password).catch((err: Error) => { });
      } else {
  ...
  public async verifyPassword(password: string): Promise<void> {
    ...
    try {
      if (passwordFieldExists) {
        await browser.$(locators.passwordFieldConfirm).click().catch((err: Error) => { });
        await browser.$(locators.passwordFieldConfirm).getValue().then(async (text: string) => {
  ...
  public async checkObfuscated(): Promise<boolean> {
    let a = false;
    await browser.$(locators.passwordFieldConfirm).waitForExist()
      .then(async (val) => {
        a = val;
