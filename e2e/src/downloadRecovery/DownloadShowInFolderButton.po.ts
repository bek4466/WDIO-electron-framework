...
import { WinExplorer } from "../WindowsExplorer/WindowsExplorer.po";
const timeout = fs.readJsonSync(path.join(__dirname, "..", "JSON", "timeout.json"));

export class DownloadShowInFolderButton extends MatButton {

    constructor(app: WebdriverIO.Browser, selector: string, private dialogSelector: string) {
        super(app, selector, "Show in folder button");
    }
