...
const datajsonfile = fs.readJsonSync(path.join(__dirname, "..", "..", "..", "tests", "regression", "Qxi_tests", "datajson", "filesQxi.json"));
...
/**
 * @Author `Amit B.QA`
 * @LinkToJIRA `https://extron.atlassian.net/browse/TOOL-3193`
 * @Description `Qxi Automation Test Suite`
 * @Date `11/18/2021`
 */
...
describe.only("Starting TestMaster spec: Running Test Suite: Qxi ", () => {
    ...
    for (const filekey in datajsonfile) {
        const datajson = fs.readJsonSync(path.join(__dirname, "..", "..", "..", "tests", "regression", "Qxi_tests", "datajson", datajsonfile[filekey]));
        // look at the keys
        // tslint:disable-next-line:forin
