...
const logClient = new LogClient("e2e:CSP-591");
/**
 * @Author `Neelam.QA`
 * @LinkToJIRA `https://extron.atlassian.net/browse/CSP-591`
 * @POM `https://extron.atlassian.net/browse/CSP-176`
 * @Description `Program Log Bad Scenario`
 * @Date `04/16/2020`
 */

describe(`${tabTitles.REGRESSION.Program_Log} CSP-591:(Program Log bad scenario)`, () => {
    before(async () => {
        ...
        allure.addLink("https://extron.atlassian.net/browse/CSP-591", "Task Issue: CSP-591");
        allure.addLink("https://extron.atlassian.net/projects/CSP?selectedItem=com.atlassian.plugins.atlassian-connect-plugin%3Acom.kanoah.test-manager__main-project-page#!/testCase/CSP-T855", "Test Case: CSP-T855");
    ...
    it("Verify that Primary controller not online gives browserropriate error message while retrieving program logs.", async () => {
        return new Promise<void>(async (resolve) => {
            ...
            systemInfo.devices[0].network.interfaces[0].address = "1.2.3.4";
            ...
            logClient.error("CSP-591: Exception Error: " + `${err}`);
...
describe(`${tabTitles.REGRESSION.Program_Log} CSP-591:(Program Log bad scenario)`, () => {
    before(async () => {
        ...
        allure.addLink("https://extron.atlassian.net/browse/CSP-544", "User Story: CSP-544");
        allure.addLink("https://extron.atlassian.net/browse/CSP-591", "Task Issue: CSP-591");
        allure.addLink("https://extron.atlassian.net/projects/CSP?selectedItem=com.atlassian.plugins.atlassian-connect-plugin%3Acom.kanoah.test-manager__main-project-page#!/testCase/CSP-T856", "Test Case: CSP-T856");
        return new Promise<void>(async (resolve) => {
            await browser.pause(10000);
            await browser.switchWindow(tabTitles.mainTab);
            await common.switchToWin(browser, tabTitles.mainTab, data.licensedUser1, data.licensedUser1pass);
    ...
    it("Verify that incorrect credentials for Primary controller gives browserropriate error message while retrieving program logs.", async () => {
        return new Promise<void>(async (resolve) => {
            const systemDeployment = new DeployComponent(browser);
            const traceComponent = new TraceComponent(browser);
            const messageComponent = new MessagePaneComponent(browser);
