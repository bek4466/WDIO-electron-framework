...
const systemInfoFileTemp = path.join(__dirname, "..", "..", "..", "resources", "XiControllerProject", "systeminfoTmp.json");
const systemInfoFilePath = path.join(__dirname, "..", "..", "..", "resources", "XiControllerProject", "systeminfo.json");
const tabTitles = fs.readJsonSync(path.join(__dirname, "..", "..", "..", "src", "JSON", "tabTitles.json"));
const data = fs.readJsonSync(path.join(__dirname, "..", "..", "..", "src", "JSON", "dataTool.json"));
const timeout = fs.readJsonSync(path.join(__dirname, "..", "..", "..", "src", "JSON", "timeout.json"));
const datajsonfile = fs.readJsonSync(path.join(__dirname, "..", "..", "..", "tests", "regression", "Schema_Cleanup", "datajson", "filesSchema.json"));
...
/**
 * @Author `Amit B.QA`
 * @LinkToJIRA `https://extron.atlassian.net/browse/TOOL-3703`
 * @Description `Path pointing outwards`
 * @Date `11/18/2021`
 */
// tslint:disable-next-line:forin
describe.only("Starting TestMaster spec: Running Test Suite: Qxi ",  () => {
    ...
    async function runTest(datajson:any, tcNum: string):Promise<void>  {
        describe.only(`${datajson.TestCaseInfo.SuiteType}":"${datajson.TestCaseInfo.UserStory}`, async () => {
            ...
            beforeEach(async () => {
                ...
                allure.addLink(UserStoryLink[0], UserStoryLink[1]);
                allure.addLink(TaskLink[0], TaskLink[1]);
                allure.addLink(TestCaseLink[0], TestCaseLink[1]);
            });
            ...
            it(updatedTestDescription, async () => {
                ...
                .catch((err: Error) => {
                    console.log("Qxi automation failed: " + `${err}`);
                })
            });
    ...
    for (const filekey in datajsonfile) {
        const datajson = fs.readJsonSync(path.join(__dirname, "..", "..", "..", "tests", "regression", "Schema_Cleanup", "datajson", datajsonfile[filekey]));
        // look at the keys
        // tslint:disable-next-line:forin
