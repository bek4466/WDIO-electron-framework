...
/**
 * @Author `Miguel.QA`
 * @LinkToJIRA `https://extron.atlassian.net/browse/TOOL-2991`
 * @Date `10/22/2021`
 */
describe(`${tabTitles.REGRESSION.Deployment} TOOL-2258 Ability for uncertified user to redeploy an un modified project that has been downloaded from the controller`, () => {
    beforeEach(async () => {
        ...
        allure.addLink("Test Case: TOOL-T564", "https://extron.atlassian.net/projects/TOOL?selectedItem=com.atlassian.plugins.atlassian-connect-plugin:com.kanoah.test-manager__main-project-page#!/testCase/TOOL-T564");
    ...
    it("verify that stray files fo not linger upon deploying a different project", async () => {
        ...
        const b = path.resolve(systemInfoFile2);
        const systemInfo2 = fs.readJsonSync(b);
        ...
        await downloadRecovery.downloadInputAddress.setDownload(systemInfo2.devices[0].network.interfaces[0].address);
        ...
        await systemDeployment.destinyInputField.setDestinyFileToUploadNoPath(downloadedProject2);
        await systemDeployment.deployButton.click()
            .then(async () => {
                await browser.pause(timeout.slow);
                return systemDeployment.deployButton.isEnabled()
...
describe(`${tabTitles.REGRESSION.Deployment} TOOL-2258 Ability for uncertified user to redeploy an un modified project that has been downloaded from the controller`, () => {
    ...
    it("verify that Certified User can Redeploy an un Modified Project that has Been Downloaded from the Controller (no Datafile)", async () => {
        await common.deleteFolderContents(downloadPath)
            .then(async () => { })
            .catch((err: Error) => {
                logClient.error("Exception caught: " + `${err}`);
