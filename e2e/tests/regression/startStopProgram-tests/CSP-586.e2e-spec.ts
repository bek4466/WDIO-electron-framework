...
const logClient = new LogClient("e2e:TOOL-1333");
/**
 * @Author `Neelam.QA`
 * @LinkToJIRA `https://extron.atlassian.net/browse/TOOL-1333`
 * @POM `https://extron.atlassian.net/browse/CSP-176`
 * @Description `Program Log Bad Scenario`
 * @Date `04/20/2020`
 */

describe(`${tabTitles.REGRESSION.StartStop_Program} TOOL-1333:(start/stop Program bad scenario)`, () => {
    before(async () => {
        ...
        allure.addLink("https://extron.atlassian.net/browse/TOOL-1333", "Task Issue: TOOL-1333");
        allure.addLink("https://extron.atlassian.net/projects/TOOL?selectedItem=com.atlassian.plugins.atlassian-connect-plugin:com.kanoah.test-manager__main-project-page#!/testCase/TOOL-T2800", "Test Case: TOOL-T2800");
    ...
    it("Verify that Primary controller not online gives browserropriate error message while starting program.", async () => {
        return new Promise<void>(async (resolve) => {
            ...
            systemInfo.devices[0].network.interfaces[0].address = "1.1.3.4";
            ...
            await browser.pause(timeout.medium);
            await messageComponent.messageRowValue.checkTroublrshootingMessagePaneLogs(data.deviceUnreachable, data.errorSeverity)
            ...
            logClient.error("TOOL-1333: Exception Error: " + `${err}`);
...
describe.only(`${tabTitles.REGRESSION.StartStop_Program} TOOL-1333:(start/stop Program bad scenario)`, () => {
    before(async () => {
        allure.addOwner("Neelam Sharma");
        allure.story("TOOL-1663:start/stop Program Bad Scenarios for Tech Preview");
        allure.addLink("https://extron.atlassian.net/browse/TOOL-1663", "User Story: TOOL-1663");
        allure.addLink("https://extron.atlassian.net/browse/TOOL-1333", "Task Issue: TOOL-1333");
        allure.addLink("https://extron.atlassian.net/projects/TOOL?selectedItem=com.atlassian.plugins.atlassian-connect-plugin:com.kanoah.test-manager__main-project-page#!/testCase/TOOL-T2801", "Test Case: TOOL-T2801");
        return new Promise<void>(async (resolve) => {
            await browser.pause(10000);
            await browser.switchWindow("ControlScript Deployment Utility");
            await common.switchToWin(browser, tabTitles.mainTab, data.licensedUser1, data.licensedUser1pass);
    ...
    it("Verify that incorrect credentials for Primary controller gives browserropriate error message while starting program.", async () => {
        return new Promise<void>(async (resolve) => {
            const systemDeployment = new DeployComponent(browser);
