...
xdescribe(`${tabTitles.REGRESSION.Login} TOOL-2253:(Profile Test: suite)`, () => {
    beforeEach(async () => {
        ...
        allure.addLink("Test Case: TOOL-T287", "https://extron.atlassian.net/projects/TOOL?selectedItem=com.atlassian.plugins.atlassian-connect-plugin:com.kanoah.test-manager__main-project-page#!/testCase/TOOL-T287");
    ...
    xit("Verify manually renewing browser license for unlicensed user", async () => {
        return new Promise<void>(async (resolve) => {
            ...
            await browser.$(ssoLocators.signInBtn)
            .click();
            await browser.pause(timeout.medium);
            await browser.switchWindow(tabTitles.main);
            await common.switchToSSOWindow(browser, tabTitles.ExtronInsider);
            await common.loginToSSO(browser, data.unLicensedUser2, data.unLicensedUser2pass);
           //  await browser.pause(timeout.slow);
            // await common.switchTobrowser(browser, tabTitles.mainTab);
            ...
            await profile.profileTitleText.checkUserName(data.unLicensedUser2)
            .then(async (value) => {
                await expect(value)
                .toEqual(true);
            })
            ...
            /* await profile.unlicenseStatusText.isClickable()
            .then(async (value) => {
                await expect(value)
                .toEqual(true);
            })
