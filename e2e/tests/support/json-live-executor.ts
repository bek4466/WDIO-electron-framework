  ...
  execFileSync(
    ...
    [
      ...
      [
        ...
        "Add-Type -TypeDefinition 'using System; using System.Runtime.InteropServices; public static class NativeWindow { [DllImport(\"user32.dll\")] public static extern IntPtr GetForegroundWindow(); [DllImport(\"user32.dll\")] public static extern IntPtr GetDlgItem(IntPtr dialog, int controlId); [DllImport(\"user32.dll\")] public static extern IntPtr SendMessage(IntPtr window, uint message, IntPtr wParam, IntPtr lParam); }'",
        ...
        '  $dialogHandle = [NativeWindow]::GetForegroundWindow()',
        '  if ($dialogHandle -eq [IntPtr]::Zero) { continue }',
        '  $confirmHandle = [NativeWindow]::GetDlgItem($dialogHandle, 1)',
        '  if ($confirmHandle -ne [IntPtr]::Zero) {',
        '    [void][NativeWindow]::SendMessage($confirmHandle, 0x00F5, [IntPtr]::Zero, [IntPtr]::Zero)',
        ...
        "  $confirmButton = @($buttons | Where-Object { $_.Current.AutomationId -eq '1' -or $_.Current.Name -match '^(Select Folder|Select|Open|OK)$' } | Select-Object -First 1)[0]",
        '} while (-not $confirmationInvoked -and $null -eq $confirmButton -and (Get-Date) -lt $deadline)',
        'if ($null -ne $confirmButton) {',
        '  $invokePattern = $confirmButton.GetCurrentPattern([System.Windows.Automation.InvokePattern]::Pattern)',
        '  $invokePattern.Invoke()',
        '  $confirmationInvoked = $true',
        '}',
        "if (-not $confirmationInvoked) { throw ('Could not find the native folder confirmation button by Win32 control ID 1 or UI Automation. Buttons found: ' + ($buttonNames -join ', ')) }",
        'Start-Sleep -Milliseconds 500',
      ].join('; '),
    ],
    {
