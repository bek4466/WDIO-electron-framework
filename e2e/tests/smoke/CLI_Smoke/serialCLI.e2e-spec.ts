...
describe(`${tabTitles.REGRESSION.Deployment} TOOL-2186:(TroubleShooting)`, () => {
    ...
    it("Verify that CLI Deploy Command Works Serially", async () => {
        ...
        return new Promise<void>(async (resolve) => {
            ...
            const firstCommand: string = await messageComponent.messageRowValue.ProcessCommandCLI("cliCommand", systemInfoFileTemp);
            const filename = "Filelog" + Date.now();
            const command = firstCommand + " && \"csdu.cmd\" --help && \"csdu.cmd\" deploy --help";
            await allure.stdOutputAttachment(command, "CLICommand");
            const stdout: any = await messageComponent.messageRowValue.CLICommandExecuter(command, filename, 0);
